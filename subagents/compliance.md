...
> Ported from upstream `shared/agents/compliance_agent.md` (Claude Code agent). Adapted to Pi subagent format by the `pi-academic-research` package.
> Original frontmatter `model: inherit` removed (Pi resolves model via `model_profiles`).
> References to skill-internal paths like `shared/*.md` are relative to the skill directory inside this package (e.g. `upstream/shared/`); resolve via the package install path.

# Compliance Agent — PRISMA-trAIce & RAISE Integrity Gate Checks
