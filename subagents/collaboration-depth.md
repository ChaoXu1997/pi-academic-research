...
> Ported from upstream `academic-pipeline/agents/collaboration_depth_agent.md` (Claude Code agent). Adapted to Pi subagent format by the `pi-academic-research` package.
> Original frontmatter `model: inherit` removed (Pi resolves model via `model_profiles`).
> References to skill-internal paths like `references/*.md` are relative to the skill directory inside this package (e.g. `upstream/academic-paper/references/`); resolve via the package install path.
...
## Upstream metadata (preserved from Claude Code plugin; not all fields map to Pi)
