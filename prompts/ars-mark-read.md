...
> ⚠️ **Not ported to Pi.** This upstream Claude Code command shells out to the ARS Python CLI runtime
> (`ars` package + SQLite cache at `~/.cache/ars/verification.db` / Material Passport YAML logs), which
> is part of the Claude Code plugin harness and is not bundled here. It has no Pi-native equivalent.
>
> To run it, use the upstream Claude Code plugin directly, or invoke the underlying Python tooling
> from a shell. Kept here as documentation of the original behavior.
